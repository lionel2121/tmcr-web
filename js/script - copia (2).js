/* 
   ...
   Archivo de Lógica (Versión Final - Calculadora Flexible y Conectada)
*/
...
// --- 5. LOGICA DE LA CALCULADORA (ACTUALIZADA: AÉREO SOLO PESO + ADVERTENCIA) ---
...
function calculateShipping() {
    ...
    // VALIDACIÓN 1: El peso es OBLIGATORIO siempre
    if (isNaN(weightInput)) {
        alert("Por favor, ingresa al menos el PESO del paquete.");
        return;
    }

    // --- CÁLCULO AÉREO (Siempre se calcula si hay peso) ---
    ...
    // --- CÁLCULO MARÍTIMO (Solo si hay dimensiones) ---
    let seaPriceUSD = 0;
    let seaPriceCRC = 0;
    let seaReason = "---";
    let isSeaCalculated = false;

    // Si el usuario puso dimensiones completas, calculamos Marítimo
    if (!isNaN(lInput) && !isNaN(wInput) && !isNaN(hInput)) {
        ...
        // Si faltan dimensiones, indicamos que se requieren
        seaReason = "Requiere Largo, Ancho y Alto";
    }
    ...
    if (isSeaCalculated) {
        ...
        document.getElementById('res-sea-detail').innerText = seaReason;
        document.getElementById('res-sea-detail').classList.add('text-red-400'); // Texto rojo para alertar
    }

    // --- NUEVO: INSERTAR ADVERTENCIA Y BOTÓN WHATSAPP ---
    // Verificar si ya existe para no duplicarlo cada vez que den clic
    ...
    if (!warningDiv) {
        ...
        `;
        // Insertarlo al final del contenedor de resultados
